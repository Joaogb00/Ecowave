  ...
  <section class="section-admin">
    ...
    <div class="conta-layout">
      ...
      <div class="acoes-container">
        ...
        <router-link to="/historico-compras" class="card-funcoes">
          <div class="card-icon"><i class="bi bi-cart-check"></i></div>
          <h3>Histórico de Compras</h3>
          <p>Veja seus pedidos realizados e status.</p>
        </router-link>
        ...
        <div v-if="papel === 'USUARIO'" class="card-funcoes card-upgrade" :class="{ 'status-pendente': solicitacaoUpgrade?.status === 'PENDENTE' }">
          <div class="card-icon">
            <i v-if="!solicitacaoUpgrade" class="bi bi-patch-check"></i>
            <i v-else-if="solicitacaoUpgrade.status === 'PENDENTE'" class="bi bi-hourglass-split text-warning"></i>
            <i v-else-if="solicitacaoUpgrade.status === 'RECUSADA'" class="bi bi-exclamation-triangle text-danger"></i>
          </div>

          <template v-if="!solicitacaoUpgrade">
            <h3>Tornar-se Proprietário</h3>
            <p>Solicite permissões avançadas para gerenciar pontos e recursos na EcoWave.</p>
            <button @click.stop="showUpgradeModal = true" class="btn-card-action">
              Tornar-se proprietário
            </button>
          </template>

          <template v-else-if="solicitacaoUpgrade.status === 'PENDENTE'">
            <h3>Upgrade em Análise</h3>
            <p class="status-msg-info">
              Sua solicitação está em análise pela administração.
            </p>
          </template>

          <template v-else-if="solicitacaoUpgrade.status === 'RECUSADA'">
            <h3>Upgrade Recusado</h3>
            <p class="status-msg-error">
              <strong>Motivo:</strong> {{ solicitacaoUpgrade.justificativa }}
            </p>
            <button @click.stop="showUpgradeModal = true" class="btn-card-action btn-retry">
              TENTAR NOVAMENTE
            </button>
          </template>
        </div>
    ...
    <div v-if="showUpgradeModal" class="modal-overlay">
      <div class="modal-box upgrade-modal">
        ...
        <h3>SOLICITAR UPGRADE DE PERFIL</h3>
        <p class="modal-intro">
          Entenda como funciona o perfil de <strong>Proprietário</strong> dentro da EcoWave:
        </p>

        <div class="upgrade-info-box">
          <div class="info-item">
            <strong>O que significa ser proprietário?</strong>
            <p>
              Permite que você assuma o controle operacional de entidades, centros de descarte ou negócios parceiros vinculados à plataforma.
            </p>
          </div>
          <div class="info-item">
            <strong>Quais são os requisitos?</strong>
            <p>
              Possuir cadastro regularizado, dados de contato válidos e passar pela checagem de conformidade da administração.
            </p>
          </div>
          <div class="info-item">
            <strong>Funcionalidades adicionais:</strong>
            <p>
              Acesso total ao painel de métricas de coleta, gerenciamento direto de resíduos recebidos e emissão/validação de pontos EcoWave.
            </p>
          </div>
        </div>
        ...
        <div class="alert-box">
          ...
          <p>
            A solicitação passará por análise minuciosa da administração antes da aprovação. O pedido não é aprovado automaticamente.
          </p>
        </div>

        <div class="modal-actions">
          <button @click="enviarSolicitacaoUpgrade" class="btn-exit-confirm" :disabled="loading">
            {{ loading ? 'PROCESSANDO...' : 'CONFIRMAR SOLICITAÇÃO' }}
          </button>
          <button @click="showUpgradeModal = false" class="btn-cancel">
            VOLTAR
          </button>
        </div>
      </div>
